...
def init_individual(frequency_for_neutral_gene, habitat_id, e, T, sexual, L_e, L_t, L_n):               #sexual为None, male, female
...
def reproduction(metacommunity, length, width, reproduction_type, birth_rate, mutation_rate, offsprings_pool, offsprings_pool_memory):
                ...
                for row in range(length):
                    for col in range(width):
                        ...
                        # 该microsite不为empty状态，并且以某一birth rate繁殖后代
                            ...
                            new_mutated_individual = mutate(individual=new_individual, mutation_rate=mutation_rate)
                            offsprings_pool.append((patch_id, habitat_id, new_mutated_individual))          # 将一个patch里的所有后代储存起来
            ...
            for habitat_id in patch:
                ...
                for parents in parents_list:
                    ...
                    if birth_rate > random_uniform_variable:
                        ...
                        new_mutated_individual = mutate(individual=new_individual, mutation_rate=mutation_rate)
                        offsprings_pool.append((patch_id, habitat_id, new_mutated_individual))
...
def immigrant_category(offsprings_pool, patch_id, habitat_id, initially_occupy_number, patch_number_each_altitude):
    ...
    for offsprings_information in offsprings_pool:
        ...
        offspring_individual = offsprings_information[2]
        if (int(patch_id[-1])-initially_occupy_number) >= 0:
            if offspring_patch_id in ['patch%d'%i for i in range(initially_occupy_number)]: 
                original_pool.append(offspring_individual)
            if offspring_patch_id == patch_id and offspring_habitat_id != habitat_id:
                local_pool.append(offspring_individual)
            if offspring_patch_id == patch_id and offspring_habitat_id != habitat_id: 
                within_pool.append(offspring_individual)
            if offspring_patch_id != patch_id and ((int(offspring_patch_id[-1])-initially_occupy_number)//3 - (int(patch_id[-1])-initially_occupy_number)//3)==0:
                among_pool.append(offspring_individual)
            if offspring_patch_id != patch_id and ((int(offspring_patch_id[-1])-initially_occupy_number)//3 - (int(patch_id[-1])-initially_occupy_number)//3)==0:
                across_pool.append(offspring_individual)
        ...
        if (int(patch_id[-1])-initially_occupy_number) < 0:
            ...
            if offspring_patch_id == patch_id and offspring_habitat_id != habitat_id:
                within_pool.append(offspring_individual)
            if offspring_patch_id != patch_id and (int(offspring_patch_id[-1])-initially_occupy_number) < 0:
                among_pool.append(offspring_individual)
...
def dispersal(metacommunity, length, width, offsprings_pool, source_rate, across_rate, among_rate, within_rate, initially_occupy_number, patch_number_each_altitude):
    a = source_rate
...
def dispersal_only_within_patch(metacommunity, length, width, offsprings_pool, within_rate):
...
def species_to_csv(metacommunity, length, width, file_name, values_type='identifier'):
    return to_csv(metacommunity, length, width, file_name, values_type)
#################################  gene_frequency ###########################################
def read_gene_frequency(metacommunity, length, width):
...
def gene_frequency_all_time_step_to_cvs(all_time_step_gene_frequency, file_name):

    columns = ['gene%d'%i for i in range(30)]
    '''此处待优化'''
...
def gene_frequency_to_csv(gene_frequency_set_start, gene_frequency_set_end, file_name):
...
offsprings_pool_memory=False                                               #布尔值，储存后代的库是否具有记忆性，及是否保存上一个time_step所产生的后代的数据
...
initial_gene_frequency = read_gene_frequency(metacommunity, length=length, width=width)
...
for time_step in range(0,25):
    ...
    offsprings_pool = reproduction(metacommunity, length=length, width=width, reproduction_type='sexual', birth_rate=1, mutation_rate=mutation_rate, offsprings_pool=offsprings_pool, offsprings_pool_memory=offsprings_pool_memory)
    ...
    all_time_step_gene_frequency=np.vstack((all_time_step_gene_frequency, read_gene_frequency(metacommunity, length=length, width=width)))
    
for time_step in range(25,50):
    print(time_step)
    dead_selection(metacommunity, length=length, width=width)
    offsprings_pool = reproduction(metacommunity, length=length, width=width, reproduction_type='sexual', birth_rate=1, mutation_rate=mutation_rate, offsprings_pool=offsprings_pool, offsprings_pool_memory=offsprings_pool_memory)
    dispersal_only_within_patch(metacommunity, length=length, width=width, offsprings_pool=offsprings_pool, within_rate=within_rate)
    local_compensation(metacommunity, length=length, width=width, offsprings_pool=offsprings_pool, initially_occupy_number=initially_occupy_number, patch_number_each_altitude=patch_number_each_altitude)
    all_time_step_gene_frequency=np.vstack((all_time_step_gene_frequency, read_gene_frequency(metacommunity, length=length, width=width)))
...
for time_step in range(50,5000):
    ...
    dead_selection(metacommunity, length=length, width=width)
    offsprings_pool = reproduction(metacommunity, length=length, width=width, reproduction_type='sexual', birth_rate=1, mutation_rate=mutation_rate, offsprings_pool=offsprings_pool, offsprings_pool_memory=offsprings_pool_memory)
    dispersal(metacommunity, length=length, width=width, offsprings_pool=offsprings_pool, source_rate=source_rate, across_rate=across_rate, among_rate=among_rate, within_rate=within_rate, 
              initially_occupy_number=initially_occupy_number, patch_number_each_altitude=patch_number_each_altitude)
    local_compensation(metacommunity, length=length, width=width, offsprings_pool=offsprings_pool, initially_occupy_number=initially_occupy_number, patch_number_each_altitude=patch_number_each_altitude)
    all_time_step_gene_frequency=np.vstack((all_time_step_gene_frequency, read_gene_frequency(metacommunity, length=length, width=width)))
...
species_to_fig(metacommunity, patch_number=patch_number, initially_occupy_number=initially_occupy_number, patch_number_each_altitude=patch_number_each_altitude, fig=fig, length=length, width=width, 
                 file_name = 'indentifier replication=%d.jpg'%1)            
termotype_to_fig(metacommunity, patch_number=patch_number, initially_occupy_number=initially_occupy_number, patch_number_each_altitude=patch_number_each_altitude, fig=fig, length=length, width=width,
                 file_name = 'termotype replication=%d.jpg'%1)           
phenotype_to_fig(metacommunity, patch_number=patch_number, initially_occupy_number=initially_occupy_number, patch_number_each_altitude=patch_number_each_altitude, fig=fig, length=length, width=width,
                 file_name = 'phenotype replication=%d.jpg'%1)            
phenotype_to_csv(metacommunity, length=length, width=width,
                 file_name = 'phenotype replication=%d.csv'%1)
termotype_to_csv(metacommunity, length=length, width=width,
                 file_name = 'termotype replication=%d.csv'%1)
species_to_csv(metacommunity, length=length, width=width,
                 file_name = 'identifier replication=%d.csv'%1)
    
finished_gene_frequency = read_gene_frequency(metacommunity, length=length, width=width)            
gene_frequency_to_csv(gene_frequency_set_start=initial_gene_frequency, gene_frequency_set_end=finished_gene_frequency, file_name='gene frequency replication=%d.csv'%1)
gene_frequency_all_time_step_to_cvs(all_time_step_gene_frequency=all_time_step_gene_frequency, file_name='all time gene frequency replication=%d.csv'%1)
